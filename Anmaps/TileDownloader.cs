                    ...
                    for (; mX <= mRightTopTile.x; mX++)
                    {
                        ...
                        for (; mY <= mRightTopTile.y; )
                        {
                            ...
                            if (mMapType == 0)
                            {
                                ...
                                {
                                    //http://online2.map.bdimg.com/tile/?qt=vtile&x={x}&y={y}&z={z}&styles=sl&udt=20190326
                                    string path = FormMain._host;
                                    path = path.Replace("{x}", mX.ToString());
                                    path = path.Replace("{y}", mY.ToString());
                                    path = path.Replace("{z}", mZoomArray[mZ].ToString());
                                    task.url = path;
                                }
